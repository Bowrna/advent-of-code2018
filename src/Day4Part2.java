...
// What is the ID of the guard you chose multiplied by the minute you chose? (In the above example, the answer would be 99 * 45 = 4455.)
    ...
    // Output variables 2 list
    HashSet<String> uniqueGuards = null;
    Map<String,List<Integer>> guardsAndPosition = new HashMap<String,List<Integer>>();

    public static void main(String[] args) {
        Day4Part2 run = new Day4Part2();
        ...
        try {
            ...
            run.putAllGuardsInSet();
            run.findGuardsPosition();
            run.output();
        } catch (Exception e) {
    ...
    public void putAllGuardsInSet()
    {
        uniqueGuards = new HashSet<>(idList);
    }
    public void findGuardsPosition()
    {
        int maxGuardId = 0;
        int totalSleptMin = 0;
        int minWhenSleptMost = 0;
        for(String guard:uniqueGuards)
        {
            Integer guardId = Integer.valueOf(guard);
            guardsAndPosition.put(guard, indexOfGuardId(guardId));
            int[] output = alwaysSleepingMin(guardId, indexOfGuardId(guardId));
            if(output[0] > totalSleptMin)
            {
                totalSleptMin = output[0];
                minWhenSleptMost = output[1];
                maxGuardId = guardId;
            }
        }
        System.out.println("Most min slet guard:"+maxGuardId+" slept max:"+totalSleptMin+" min slept most:"+minWhenSleptMost);
        System.out.println("Part 2 output:"+maxGuardId* minWhenSleptMost);
    }
    ...
    public void output() {
        System.out.println("mostSleptGuardId:"+mostSleptGuardId+" mostSleptMinute:"+mostSleptMinute);
        System.out.println("Output is:" + mostSleptGuardId * mostSleptMinute);
    }

    public void indexOfMaxSleptGuard() {
        // System.out.println("idlist:" + idList);
        positionOfMostSleptGuard = indexOfGuardId(mostSleptGuardId);
    }
    public List<Integer> indexOfGuardId(Integer guardId){
        List<Integer> guardIdList = new ArrayList<Integer>();
        for(int position = 0; position < idList.size();position++)
        {
            if (Integer.valueOf(idList.get(position)).equals(guardId)) {
                guardIdList.add(position);
            }
        }
        return guardIdList;
    }
    public int[] alwaysSleepingMin(Integer guardId, List<Integer> guardPosition)
    {
        int[] out = new int[2];
        int[] mins = new int[60];
        for (int index = 0; index < guardPosition.size(); index++) {
            int pos = guardPosition.get(index);
            for (int j = 0; j < 60; j++) {
        ...
        out[0] = max;
        out[1] = maxposition;
        System.out.println("Guard:"+guardId+" slept max of:"+max+" mostly at minute:"+maxposition);
